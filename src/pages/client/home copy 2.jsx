        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {categories.map((cat) => (
            <Link to={`/products?category=${cat.name}`} key={cat.categoryId} className="bg-[#fdf6f0] border rounded-xl shadow hover:shadow-lg transition overflow-hidden">
              <img src={cat.image || "/images/default-category.jpg"} alt={cat.name} className="w-full h-120 object-cover" />
      ...
      <section className="bg-[#fdf6f0] py-16 px-4 sm:px-10 text-center max-w-7xl mx-auto">
        ...
        {allReviews.length === 0 ? (
          ...
          <div className="relative">
            <button onClick={scrollLeft} className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-white shadow p-2 rounded-full z-10">
              <FaChevronLeft />
            </button>
            <div
              ref={scrollRef}
            ...
            >
              {allReviews.slice(0, 20).map((review) => (
                <div key={review.reviewId} className="bg-white p-6 rounded-xl shadow flex-shrink-0 w-72">
            ...
            <button onClick={scrollRight} className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white shadow p-2 rounded-full z-10">
              <FaChevronRight />
            </button>
          </div>
        )}
