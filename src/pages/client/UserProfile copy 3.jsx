...
const handleProfileUpdate = () => {
  ...
  axios
    ...
    .then((res) => {
      ...
      setUser(res.data.user); // <-- Use updated user from backend
      setEditProfile(false);
    })
    .catch(() => toast.error("Failed to update profile"));
};
  ...
  return (
    <div className="min-h-screen bg-[#fdf6f0]">
        ...
        <aside className="bg-white rounded-xl shadow-lg p-6">
          ...
          {!editProfile ? (
            <div className="space-y-3 text-[#802549] text-base">
              ...
              <button className="mt-4 text-sm text-pink-600 underline" onClick={() => setEditProfile(true)}>
                Edit Profile
              </button>
              <button className="block text-sm text-pink-600 underline" onClick={() => setShowPasswordModal(true)}>
                Change Password
              </button>
            </div>
          ) : (
            <div className="space-y-3">
              ...
              <div className="flex justify-end space-x-2">
                <button className="bg-pink-600 text-white px-4 py-2 rounded" onClick={handleProfileUpdate}>
                  Save
                </button>
                <button className="text-gray-500 underline" onClick={() => setEditProfile(false)}>
                  Cancel
                </button>
              </div>
            </div>
      ...
      {/* Order Details Modal */}
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50" onClick={() => setSelectedOrder(null)}>
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl relative" onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-4 right-6 text-2xl text-pink-600" onClick={() => setSelectedOrder(null)}>
              &times;
            </button>
            <h3 className="text-xl font-bold mb-4 text-[#802549]">Order Details - {selectedOrder.orderId}</h3>
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50" onClick={() => setShowPasswordModal(false)}>
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end space-x-3">
              <button className="bg-pink-600 text-white px-4 py-2 rounded" onClick={handlePasswordChange}>
                Save
              </button>
              <button className="text-gray-500 underline" onClick={() => setShowPasswordModal(false)}>
                Cancel
              </button>
            </div>
          </div>
